...
class MainPage extends React.Component {
  ...
  calculateCategorySum(category, data) {
    let categorySum = 0;
    data.forEach((item) => {
      if (item.category === category) categorySum += item.itemCost;
    });
    return categorySum;
  }

  getItemsByCategory(data, categories) {
    let modelData = Object.assign(
      {},
      ...categories.map((key) => ({ [key]: { items: [], total: 0 } }))
    );
    data.forEach((item) => {
      ...
      <div className='main-wrapper'>
        {categories
          ? categories.map((category, index) => {
              let categorySum = this.calculateCategorySum(
                category,
                this.props.items
              );
              return (
                <table key={index}>
                  ...
                  <tbody>
                    <ItemDetail
                      itemsData={itemsByCategory[category].items}
    ...
    return (
      <div className='main-container'>
        <div className='main-heading'>
          <button onClick={this.handleOpenModal} className='add-btn'>
            Add Item
          </button>
          <div className='main-total'>
            <h1>Total: {this.calculateTotalSum(this.props.items)}</h1>
          </div>
        </div>
